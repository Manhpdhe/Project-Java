...
<html>
    ...
    <body>
        ...
        <form action="${pageContext.request.contextPath}/headmanage/movie/update" method="post" enctype="multipart/form-data">
            ...
            <br> Rate: <select id ="language" name="languageID">
                <c:forEach items="${requestScope.languageList}" var="language">
                    <option value="${language.getLanguageId()}" 
                            <c:if test="${language.getLanguageId() eq requestScope.movieUpdated.getLanguage().getLanguageId()}">
                                selected="selected"
                            </c:if>>${language.getLanguageName()}</option>
                </c:forEach>
            </select>   
            <br> View: <input type="text" name="view" value="${requestScope.movieUpdate.getView()}">
            <br>Video Movie: <input type="text" name="url" value="${requestScope.movieUpdate.getURL()}">
